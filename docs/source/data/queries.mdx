...
Will automatically refetch data when underlying field values change. If the query is defined in a mutation's `refetchQueries`, it will be ignored (no matter of the `fetchPolicy`) and a race condition against the mutation will occur.
...
Will not update on cache changes.

Will not execute without calling `refetch` or `updateQueries`.

Will _not_ automatically refetch data when underlying field values change.
